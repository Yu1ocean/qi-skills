...
- **【技能说明文档重复 ZIP 块系统性缺陷（2026-08-21 巡检）】**：forge 流水线每次发布都在说明文档追加新 ZIP 文件块，但**从未清理旧块**，导致「幽灵安装包」堆积。全库扫描 24 篇说明文档（wiki 别名已去重）：7 篇存在重复 ZIP 块，最严重 info-miner（docx `Q9CJdIyEUoeJMZxT5eGcnE6JnrH`，wiki 别名 `NvL8wxQIui73ZrkJh2DcINfKnRb`）堆了 **14 个** ZIP；team-travel（docx `WsR7dA8ZpoNqIExN8AHcvsBTnKe`，wiki 别名 `Ba0pwnbXUiw9C1kWzOxcc6fYn8c`）3 个共 ~5.2MB。另有 3 篇 0 ZIP 块（缺交付锚点）：zero-trust-data-analyzer / v6-panoramic-chart-generator / cyber-inspiration-generator(DVRed 旧版)。
  - **根治方向**：forge 的 ZIP 回挂步骤应改为「先扫同名 ZIP 块 → block_delete 旧块 → 再插入新块」的 upsert 语义，而非纯 append。
