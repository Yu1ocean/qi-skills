## 📌 技能简介
将 UK/EU/JP POP BD 团队的差旅审批邮件与商旅预订通知自动沉淀为结构化差旅资产：默认全量抓取可识别姓名的邮件，兼容单程票 / 多段链式行程，输出带【合规健康度雷达】、【今日新增预警】、🆕 NEW 新增预警高亮、费用主卡常驻展示与强提醒特效的差旅大屏 V3.6，并补充 `output/mail_ledger.json` 作为 Email Logger 零信任 QA 审计台账。适用于团队差旅巡检、周会大屏、跨区域拜访排期复盘与合规风险扫描场景。
...
- **今日新增预警**：每次生成 JSON 后写入 `output/snapshots/YYYY-MM-DD.json`，并基于昨日快照对 `compliance.alerts` 做集合差；唯一键为“人员姓名 + 预警类型 + 日期区间”三元组，新增项会写入 `is_new=true` 并在 UI 与飞书卡片中显示 `🆕 NEW` 高亮。
- **飞书卡片模板**：`assets/team_travel_dashboard_card_template.json` 为官方兜底卡片模板，必须使用 `schema: 2.0` 与 `body.elements`；通过 `scripts/build_travel_card_payload.py` 从快照生成 `.ephemeral_pool/[task_id]_team_travel_dashboard.card.json` 后，再交由 `centralized-transmitter` 创建与发送。
