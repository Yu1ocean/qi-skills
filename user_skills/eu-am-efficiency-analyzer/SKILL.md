...
- **改名类变更（Sheet / 视图重命名）只改了文档与线上标签页，没有同步 `build_layered_sheets.py` 的 `SHEET_*` / `VIEW_*` 常量** —— `+table-put` 会按旧名静默新建一批重复 Sheet，数据写错位置且无报错（v1.2.1 改名后的真机事故来源）。
...
| 底表 | `【2.AM底表】` | `8953af` | 1422 | 106 | `AM优先级 == "AM招商推进" & 历史入驻 != 1`（v1.2.2 起追加剔除历史入驻商家） |
| 底表 | `【3.BD底表】` | `MpyNOP` | 1030 | 106 | `负责BD` 非空（非 None / 非空串 / strip 后非空） `& 历史入驻 != 1`（v1.2.2 起追加剔除历史入驻商家） |
| 阅读 | `【1.全量看板】` | `KYImDl` | 8496 | 38 | 引用 `【1.全量底表】` |
| 阅读 | `【2.AM看板】` | `t5m7r4` | 1422 | 38 | 引用 `【2.AM底表】` |
| 阅读 | `【3.BD看板】` | `JC5aOe` | 1030 | 38 | 引用 `【3.BD底表】` |
| 🔒 受保护 | `历史入驻` | `Tc3dvL` | — | — | **只读**，任何写入/clear 立刻熔断 |
| 🔒 受保护 | `AM分析` | `M45mLI` | — | 38 | **只读**（原名「分析基盘_阅读视图」，线上已被人工改名） |

\* v1.2.3 真机同步实测：`【2.AM底表】` 1422 行、`【3.BD底表】` 1030 行（`【1.全量底表】` 8496 行）。自 v1.2.2 起，`【2.AM底表】` / `【3.BD底表】` 追加筛选条件 `历史入驻 != 1`（剔除历史入驻商家），实际行数以下次同步的 `layered_result.json` 为准（剔除前分别为 1906 / 1480 行）；`【1.全量底表】` 保留全量快照语义，不加任何新筛选。
...
- **v1.2.3**：修复真机分层同步暴露的两个缺陷。① **P0 · Sheet 名未随 v1.2.1 改名同步**：`build_layered_sheets.py` 的常量仍是旧名（`全量底表` / `全量_AM招商推进` / `BD底表` / `全量_阅读视图` / `AM招商推进_阅读视图` / `BD_阅读视图`），导致 `+table-put` 静默新建 6 张同名重复 Sheet、数据写错位置；现统一改为 `【1.全量底表】` / `【2.AM底表】` / `【3.BD底表】` / `【1.全量看板】` / `【2.AM看板】` / `【3.BD看板】`。② **P1 · QA 阈值未按 G7 分表配置**：`qa_null()` 新增 `null_cols` 参数，主流程仅对 `SHEET_AM` 传 `QA_NULL_COLS`，`【1.全量底表】` / `【3.BD底表】` 传空列表并输出「负责AM/AM优先级 空值率断言按 G7 豁免（源数据事实，非 FAIL）」；`【3.BD底表】` 的 `负责BD` 非空率 == 100% 硬断言保留。真机验证：三底表 8496 / 1422 / 1030 行，历史入驻=1 回读分别 1829 / 0 / 0，`overall == true`。同步补充 Red Flags：改名类变更必须同步 `SHEET_*` / `VIEW_*` 常量。
- **v1.2.2**：AM底表、BD底表新增筛选条件「历史入驻 != 1」，剔除历史入驻商家；全量底表不变。（实现：`filter_records()` 新增 `is_hist_settled()` 判定，经 `norm_scalar` 归一化，兼容数字 `1` / 浮点 `1.0` / 字符串 `"1"` / 带空格 `" 1 "` / 布尔 `True` 五种存储形态；`【1.全量底表】` 保留全量快照语义，不加任何新筛选。）
