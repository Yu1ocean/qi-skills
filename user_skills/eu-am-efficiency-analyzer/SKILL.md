...
| 底表 | `【2.AM底表】` | `8953af` | 动态 | 106 | `AM优先级 == "AM招商推进" & 历史入驻 != 1`（v1.2.2 起追加剔除历史入驻；口径演进：1422 → 1906 → 剔除历史入驻后动态） |
| 底表 | `【3.BD底表】` | `MpyNOP` | 动态 | 106 | `负责BD 非空（非 None / 非空串 / strip 后非空） & 历史入驻 != 1`（v1.2.2 起追加剔除历史入驻） |
